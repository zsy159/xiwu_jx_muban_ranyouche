"""Load finance UI inputs from Streamlit 邀约专员算薪 page."""
...
INPUTS_FILENAME = "invite_specialist_inputs.json"
...
def load_finance_hub_overrides(month: str) -> dict[str, float]:
    ...
    try:
        ...
        logger.warning("cannot read invite finance inputs %s: %s", path, exc)
        return {}
    ...
    if overrides:
        logger.info("invite finance overrides: %s names from %s", len(overrides), path)
    return overrides
