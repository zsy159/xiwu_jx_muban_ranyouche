...
def _cached_perf_frame(month_id: str, sales_mtime_ns: int) -> pd.DataFrame:
    del sales_mtime_ns
    cfg = load_month_config_for(month_id)
...
@st.cache_data(show_spinner=False)
def _cached_eval_perf(month_id: str, sales_mtime_ns: int) -> pd.DataFrame:
    sales_path = _sales_workbook_path(month_id)
    loader = _cached_loader(str(sales_path), sales_mtime_ns)
    perf = _cached_perf_frame(month_id, sales_mtime_ns)
...
def get_eval_perf_frame(month_id: str) -> pd.DataFrame | None:
    ...
    if not path.exists():
        return None
    return _cached_eval_perf(month_id, _mtime_ns(path))
...
def get_computed_perf_frame(month_id: str) -> pd.DataFrame | None:
    ...
    if not path.exists():
        return None
    return _cached_perf_frame(month_id, _mtime_ns(path))
