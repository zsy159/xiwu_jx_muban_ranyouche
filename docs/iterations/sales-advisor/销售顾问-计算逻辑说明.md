...
车型（H）+ 销售渠道（I）+ 指标汇总部门（A）
    ...
    → 按顾问姓名（P）SUM = 单车绩效合计
    → × 完成率（个人块 ×H，封顶按拓扑 120% 或 110%；门店块 ×BA，读任务表 AG 总完成率，**不在系统内再封顶**）= Hub 整车绩效（W）
```
...
| `IF(G/F>110%, 110%, G/F)` | **110%** | 少数 Hub 行 H 列公式（非 BA） |

门店块整车绩效乘 **BA**：值为任务表 AG 列已算好的总完成率（可出现 120%），系统 `lookup_combined_completion_rate` **不再套封顶**。
